	...
	$(function() {
		...
		teamCheck.change(function() {
			...
			teamCheck.each(function() {
				if ($(this).is(':checked')) {
					if ($(this).val() != teamVal && teamVal != null) {
						alert('하나의 주문내역에서만 체크 가능합니다.');
...
<form action="ready" method="get" id="orderSearchForm">
	<div id="content">
		<div class="head_tit">
			<h1>배송 관리</h1>
			<h5 id="d_tit" style="display: none">ready</h5>
		</div>
		...
		<div class="section" id="">
			...
			<div id="tabNumber" class="tabCont">
				...
				<div class="ctrl">
					<a href="#" id="btnDetach" class="btnMini"><span>개별배송처리</span></a>
				</div>
											...
											<tr>
												...
												<c:if test="${status.first }">
													<td class="w140 left t_td" style=""
														rowspan=${status.first? orderListVO.getCount():""}><select
														class="fSelect codeSelect" name="t_code"
														style="width: 110px;">
															<c:forEach var="transportDto" items="${transportDto }">
																<option value="${transportDto.id}">${transportDto.name}</option>
															</c:forEach>
													</select> <input type="text" name="t_invoice" class="fText invText"
														value=""> <a href="#" class="btnMini btnInvoice"><span>송장번호저장</span></a></td>
												</c:if>
