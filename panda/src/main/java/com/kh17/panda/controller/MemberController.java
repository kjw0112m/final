...
// 회원가입	
	...
	public String regist(@ModelAttribute MemberDto memberDto) {
		...
		if (result) {
			pointDao.save(PointDto.builder().type("적립").use_point(1000).current_point(1000)
					.content("회원가입 축하 보너스").member_id(memberDto.getId()).team("").build());
...
//	목표 : 입력페이지로 전달
	@GetMapping("/new_pw")
	public String newPassword(@RequestParam String email, @RequestParam String id, @RequestParam String no,
...
//	목표 : 변경 처리 수행
	@PostMapping("/new_pw")
	public String newPassword(@ModelAttribute MemberDto memberDto, HttpServletRequest request,
...
//아이디 찾기 기능
	...
	public void change_pw(HttpSession session, @ModelAttribute MemberDto memberDto, Model model,
			@RequestParam String new_pw, HttpServletResponse resp) throws IOException {
		resp.setContentType("text/plain");
...
//		[3] 비밀번호가 다르면 비밀번호 변경 실패 안내
	...
	public void verification(@RequestParam String email, HttpServletResponse resp)
	...
	public void email_verification_check(@RequestParam String identity, HttpServletResponse resp) throws IOException {
		resp.setContentType("text/plain");
