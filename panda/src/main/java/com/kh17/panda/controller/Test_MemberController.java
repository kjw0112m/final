...
// 사용 경로 : http://localhost:8080/panda/seller/notice/noticeUpdate
...
public class Test_MemberController {
...
// 비밀번호 암호화처리
		...
		if(result!=null) {
			if(BCrypt.checkpw(memberDto.getPw(), result.getPw())) {
				...
				session.getAttribute("sid").toString().startsWith("!");
				...
				return "redirect:/seller/notice/noticeList";
			}else {
